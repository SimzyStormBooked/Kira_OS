...
export function BookMetadataForm({ book, series, onSaved, onCancel }: {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const saved = libraryBookSchema.parse((response as { book: unknown }).book); await reload(); onSaved(saved);
  ...
  return <form className="library-form" onSubmit={submit} aria-busy={busy}>
    ...
    <div className="library-actions"><Button type="submit" disabled={busy}>{busy ? "Saving…" : book ? "Save book details" : "Add book"}</Button><Button type="button" variant="ghost" disabled={busy} onClick={onCancel}>Cancel</Button></div>
  </form>;
}
...
export function ConnectedLibrary() {
  ...
  return <div className="library-page">
    ...
    <Card className="library-intro"><BookOpen size={25} /><div><h2>Your words become Kira’s reference.</h2><p>Add a manuscript. Kira reads it in small steps, saves what it learns, and shows you the passages behind each finding. You decide what belongs in your marketing.</p><span className="quiet-note">Private to your workspace. Your manuscript is never rewritten.</span></div></Card>
    {loading && !data && <p role="status">Opening your book library…</p>}
    ...
    <Dialog open={adding} onOpenChange={setAdding}><DialogContent className="library-dialog"><DialogHeader><DialogTitle>Add a book</DialogTitle><DialogDescription>A title is enough to start. You can add details and a manuscript later.</DialogDescription></DialogHeader><BookMetadataForm series={series} onCancel={() => setAdding(false)} onSaved={book => { setAdding(false); router.push(`/universe/${book.slug}`); }} /></DialogContent></Dialog>
  </div>;
}
