...
export function BookDetailsBrief({ book, seriesName }: { book: LibraryBook; seriesName: string }) {
  ...
  return <><Button ref={button} variant="outline" type="button" disabled={!ready || busy || !canEdit} onClick={() => { if (scratchpad.title.trim() || scratchpad.draft.trim()) setChoice(true); else prepare(); }}>Prepare book details</Button>
    <Dialog open={choice} onOpenChange={setChoice}><DialogContent onCloseAutoFocus={event => { event.preventDefault(); button.current?.focus(); }}><DialogHeader><DialogTitle>Keep your unfinished idea?</DialogTitle><DialogDescription>You already have words at your desk. Keep those words, or replace the unsaved draft with a starter for this book.</DialogDescription></DialogHeader><DialogFooter className="book-materials-choices"><Button variant="ghost" onClick={() => setChoice(false)}>Stay with this book</Button><Button variant="outline" onClick={() => { setChoice(false); router.push("/desk"); }}>Open my current brief</Button><Button onClick={prepare}>Replace with book details</Button></DialogFooter></DialogContent></Dialog>
  </>;
}
