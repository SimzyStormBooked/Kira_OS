...
const fields = [["role", "Role"], ["description", "About"], ["physical_traits", "Physical traits"], ["backstory", "Backstory"], ["archetype", "Character archetype · interpretive"], ["character_tropes", "Character tropes · interpretive"], ["emotional_growth", "Emotional growth"], ["personality", "Personality"], ["relationships", "Relationships"], ["arc", "Story development"], ["marketing_description", "Possible marketing angle · review before use"]] as const;
export function BookKnowledge({ knowledge, citations }: {
  ...
  return <div className="book-knowledge">
    ...
    <div className="library-actions knowledge-views" role="group" aria-label="Explore book knowledge">
      {([ ["characters", "Character Organization"], ["story", "Story Arc"], ["readers", "Marketing"] ] as const).map(([key, label]) => <Button key={key} type="button" variant={view === key ? "default" : "outline"} aria-pressed={view === key} onClick={() => setView(key)}>{label}</Button>)}
    </div>
    {view === "characters" ? <>
      <h3 className="library-subheading">Characters, with their sources</h3>
      <p className="quiet-note">One card per name or explicit matching alias in this manuscript. New readings can organize physical traits, backstory, archetypes, character tropes and emotional growth; older readings may not have separate entries yet. Open a profile to explore its observations. These are unreviewed findings, not an author-approved character bible.</p>
      ...
      <div className="library-facts">{found.map(group => <Card className="library-fact knowledge-character" key={group.name}>
        ...
        <details><summary>Explore {group.name} · {group.observations.length} source observation{group.observations.length === 1 ? "" : "s"}</summary>
          ...
          {fields.map(([field, label]) => {
            ...
            if (!observations.length) return <details className="knowledge-field" key={field}><summary>{label}</summary><p className="quiet-note">No separate observation recorded for this category. It may be covered in About or the original source; KIRA will not fill the gap by guessing.</p></details>;
      ...
      <div className="library-facts">{facts.map(group => <Card className="library-fact knowledge-fact-group" key={group.id}>
        ...
        <details><summary>Explore {group.facts.length} finding{group.facts.length === 1 ? "" : "s"}</summary>{group.facts.map((fact, index) => <div className="knowledge-observation" key={index}>
          <span className="eyebrow">{factLabels[fact.category]}</span><span className={`library-fact-label ${fact.kind === "inference" ? "inference" : ""}`}>{fact.kind === "supported" ? "Manuscript-supported · unreviewed" : "Marketing / interpretive inference"}</span><p>{fact.statement}</p>{citations(fact)}{view === "readers" && <details><summary>Review source excerpt candidates</summary><p className="quiet-note">These are original source quotations for private review. Check spoilers, context and publication rights before using one in an ad or teaser.</p>{fact.citations.map((citation, n) => <blockquote key={n}>{citation.quote}</blockquote>)}</details>}
        </div>)}</details>
      </Card>)}</div>
    </>}
