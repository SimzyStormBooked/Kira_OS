...
test("learned knowledge has private citations, hides spoilers, searches text, and retains the good version during replacement",async({page,request},testInfo)=>{
  ...
  await expect(page.getByRole("heading",{name:"Knowledge ready",exact:true})).toBeVisible();await expect(page.getByText("Manuscript-supported · unreviewed",{exact:true}).first()).toBeVisible();
  const hidden=page.getByText(/findings with potential spoilers are hidden/);await expect(hidden).toBeVisible();await expect(page.locator(".library-fact")).toHaveCount(3);
  await page.getByLabel("Reveal plot details and potential spoilers",{exact:true}).check();await expect(page.locator(".library-fact")).toHaveCount(4);
