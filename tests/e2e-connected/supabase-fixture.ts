  ...
  try {
    ...
    if (url.pathname === "/__test/manuscripts" && request.method === "POST") {
      ...
      const profile = validateManuscriptExtraction({
        facts: [
          ...(body.extended === true ? Array.from({length:8},(_,i)=>({category:"theme",statement:`Synthetic extended theme ${i+1}: ${i===7?"late-match-signal":"reference observation"}.`,kind:"supported",spoiler:false,citations:[citation]})) : []),
    ...
    if (url.pathname.startsWith("/rest/v1/rpc/") && request.method === "POST") {
      ...
      if (method === "ads_control") {
        ...
        if (body.p_action === "link") { adsBookLinks=adsBookLinks.filter(item=>item.campaign_id!==payload.campaignId); if(payload.bookId) adsBookLinks.push({author_id:fixture.authorId,campaign_id:String(payload.campaignId),book_id:String(payload.bookId)}); return respond(response,200,{}); }
