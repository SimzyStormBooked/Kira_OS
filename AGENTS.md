...
Ask Raven now has an on-demand Gateway implementation behind `runStudioProvider`, separate from demo synthesis. It supports bounded business brainstorming, agent-idea design, and learning with fixed system policy, strict structured output, and no external-action tools. A UUID/pending record precedes the model call; outcomes retain model, usage/cost, actor, and sanitized status at private addressable URLs. SQL enforces caller roles, a server-only recording capability, idempotency, one pending request, and a shared daily limit. The capability hash stays in locked private configuration; public RPC wrappers are invokers and privileged implementations stay in the private schema. Keep live generation disabled until funded and verified; never substitute demo output or claim a blueprint is running.

Connections distinguish manual shortcuts from owner-consented Meta authorization. The Meta implementation stores encrypted credentials privately and requires its own server capability, scoped read consent, and verified account state; credentials and successful hosted consent are still required for activation. It does not yet ingest social metrics or publish. The NotebookLM bridge copies only user-selected text for a person to paste; it does not automatically transfer documents. Access management applies only to existing confirmed accounts and sends no invitations. None of these additions relaxes the creative firewall or the requirement for explicit external-action authorization.
