...
`npm run check` passed TypeScript, ESLint, **637 unit/API/PostgreSQL tests across 46 files**, and the production build. The PostgreSQL suite includes recovery success/failure, replay, pause, permissions, daily allowance, run binding, both sibling completion orderings and the recovery budget across jobs. It exercises actual SQL with PGlite; it does not simulate separate server connections competing for a lock. The worker tests cover ambiguous reservation replies without a second paid generation. Provider diagnostics retain only fixed error categories, bounded allowlisted schema fields and safe generation IDs. All **8 connected desktop/mobile manuscript browser tests** passed, including upload permission, queued work, private citations, spoiler controls, search and version preservation.

Migration `202609230001_manuscript_batch_recovery.sql` was applied at **2026-09-23 22:16:47 UTC**, under the shared migration advisory lock and author-row locks, using verified TLS. Both batch-lineage and canonical passage-set uniqueness indexes were verified before commit. There were zero pending manuscript batches and zero active reading jobs. This adds recovery controls without changing saved manuscripts or weakening citation validation. See [the sanitized migration record](docs/manuscript-recovery-hosted-checkpoint.json). The application deployment `433c70a` became ready before the existing manuscript resumed at 232 of 421 passages.

At **2026-09-23 22:27 UTC**, the production job completed **421 of 421 distinct passages**. All 421 have their optional search embeddings, book knowledge is active, the job is complete and no batch remains pending. Three invalid responses (unsupported citations or a citation-count schema failure) each received one separately recorded automatic recovery and succeeded. Authenticated production reads verified the completed book page, a saved quotation against its supporting passage, and passage search; anonymous passage access returned 401. This confirms the deployed recovery path as well as local tests. See [the live checkpoint](docs/manuscript-recovery-live-checkpoint.json). Findings remain unreviewed model interpretations; complete passage processing does not claim perfect semantic recall.
...
This integration combines Character Studio's gallery, profile view and sanitized private portrait uploads through `1f5e8ca` with the UX branch through `8c0d9be` (including the `82e0320` draft safety, contrast, manuscript custody and honest-state fixes). No third-round score has been commissioned. Editing character notes and book links, the home showcase and Quiet Room remain future work. Automatic emails remain deferred and Meta Ads still requires app setup and account consent.


## Integrated release checks — September 21, 2026

The release combines main through `1f5e8ca` and the UX branch through `8c0d9be`. Character Studio now appears in available navigation rather than the planned-module section; a connected desktop/mobile test reaches it through that navigation before creating and searching an alias. Available and planned navigation lists are explicit, avoiding position-dependent classification when pages are added.

Meta's app-wide deauthorization and deletion callbacks accept either the organic or Ads login configuration, while organic OAuth still requires its own login configuration. Focused tests cover Ads-only and organic-only revocation, signed deletion receipts, invalid signatures/configuration, and retryable storage failures. The existing server capability and credential key are preserved. This fixes application setup behavior; a real Meta app and account consent are still required.

`npm run check` passes TypeScript, ESLint, **568 unit/API/database tests in 45 files**, and the production build. The earlier merged demo suite passed all **20 desktop/mobile tests**, including Character Studio accessibility. The final connected suite passed all **122 desktop/mobile tests** against the local simulated Supabase boundary, including the new navigation regression and private portrait lifecycle. This is local end-to-end verification, not a claim of live Meta authorization or a hosted portrait write. No paid generation, ad mutation, or email delivery was initiated by this verification.

## Historical checkpoint — September 20, 2026
...
That separate database check used TLS without verifying the certificate chain. It also could not identify the migration applier at the time. The integration provenance above now resolves that uncertainty: this Codex session applied the migration explicitly over verified TLS and saved its result. There is no evidence of a merge-triggered migration runner; merging application code should not be treated as applying SQL.
...
The gallery and profile view were added afterwards at `/characters` and `/characters/[id]`, with `GET`/`POST /api/characters` and `GET`/`PATCH /api/characters/[id]`. Portraits are read through five-minute signed URLs; no `storage_path` field is exposed, and the path appears only inside an expiring signed URL, which is what Storage signs. 23 new unit tests cover the sanitizer, the upload route, and the gallery routes; the full `npm run check` passes with 548 tests across 44 files, and the 20-test demo browser suite passes, now including `/characters` in the automated accessibility sweep with zero violations on desktop and mobile.
...
`tests/e2e-connected/characters.spec.ts` runs the real interface against the simulated Supabase boundary, five tests on each of desktop and mobile, bringing that suite to **122 passing**. It creates a character, finds her again by alias, uploads a portrait, refuses an image that cannot be cleaned, requires a source credit before promotional use, chooses a cover, and reads a confirmed book link. Its assertions are about what reached private storage: no location metadata in the stored bytes, a hash matching the registered hash, and an image that decodes at its true dimensions through an expiring signed link.

Extending the fixture caught two defects that the unit tests could not:

1. **Both gallery reads were origin-gated.** `GET /api/characters` and `GET /api/characters/[id]` called `assertSameOrigin`. Browsers omit `Origin` on a same-origin GET, so every read returned 403 and the client bounced to the sign-in screen. The unit tests passed because they set an `Origin` header by hand. The reads now match `/api/library`: the verified session and RLS are the boundary, and writes remain origin-checked. A regression test asserts a read succeeds with no `Origin` header.
2. **A signed link contains the object path.** An earlier claim that "no storage path appears in any response" was wrong: Storage signs the path, so it necessarily appears inside the URL. The accurate guarantee, now asserted, is that no `storage_path` field is exposed and the path appears only inside a signature that expires. The documentation has been corrected.

Still absent: the hosted workspace path. No image has ever been stored in the hosted bucket, and notes and book links are read-only in the interface. The simulated boundary is a transport double, not a database: SQL and RLS are proven separately by the PGlite tests. Schema, a route, and a rendered page are not the same as a verified working feature.
