...
“Use this idea” navigates with an allowlisted idea ID, never private draft text in the URL. `ManualReviewForm` resolves known IDs into editable starters and requires an explicit save. A provider-owned scratchpad preserves unfinished words across internal navigation; replacing existing words requires a choice. Scratchpad fields are outside `workspaceSchema`, so they never enter the demo localStorage key, Supabase persistence, or workspace exports. They are mirrored into per-tab `sessionStorage` under a mode- and account-scoped key and validated with Zod on restore, so a reload of the same tab returns them; unreadable or unavailable storage is ignored. Sign-out discards them, and a before-unload prompt warns while a draft remains. Only a submitted brief enters the ordinary saved-review flow.

`/learn` combines collapsed optional lessons with four curated agent recipes and a direct jump to the form. Name, goal, context, and success criteria produce a local text blueprint. Copying/downloading is explicit; “Save blueprint to my desk” uses the existing manual-review operation. Per-recipe notes, generated previews, and save/download signatures live in provider-owned state, along with the unsent Studio question, its retry identity, and unsaved rewrites of a saved brief (keyed by brief and draft number). They survive internal navigation and a reload of the same tab, stay outside persistence/export schemas, and reset on account change. A detected session loss no longer clears them: the provider marks the session ended, and a blocking dialog shows every unfinished draft read-only with a copy control; the scratchpads and their sessionStorage entry are removed only when she chooses to sign in again. A shared before-unload check covers all three scratchpads. Explicit sign-out confirms discarding unfinished work and retains it if sign-out fails. Viewers can learn and export a blueprint without gaining write access; no recipe starts an agent.
...
`KIRA_AI_ENABLED=true`, valid recording configuration, and positive verified Gateway credit are required before submission. No demo/offline answer substitutes for an unavailable provider. These gates are configured in production. A real Gemini 3.8 Flash request through the deployed UI completed, survived reload, and was explicitly copied into an attributed pending Desk brief. The generation retains token usage and estimated cost; sign-out closed private API access. This verifies application persistence as well as provider connectivity without enabling background agents or external actions.
...
Hosted Supabase provisioning, the core sign-in/save/reload/separate-browser/sign-out workflow, and real hosted AI generation persistence are verified. The adversarial UX fixes are deployed in release `562c108`; 336 unit/API/database tests and 94 browser tests pass. Real Meta consent remains unverified and its app setup is incomplete. [SETUP.md](SETUP.md) covers use and remaining setup; [VERIFICATION.md](VERIFICATION.md) records the precise hosted checks and their limits.
